...
file:PhilaJ-setup-measure-menu.gif

The following dialog box will appear:

#+ATTR_HTML: :align center :width 400px
file:PhilaJ-setup-measure-dialog.gif
...
#+ATTR_HTML: :align center :width 400
file:PhilaJ-dialog-sheetSep-Options.gif
...
are used to label the stamp images with the position number from the original sheet from which the block came:
 ...
 - The width of the *sheet* from which the block came -- if you don't know, then leave set it to the number of columns in the block
...
original sheet.  If you don't know what values to use for sheet position, set the first to "=1=" and the second to the width of the block -- this will number
the stamps consecutively starting at 1. The remaining options are used to specify how the image overlay will look -- in this case we have selected pretty wide lines (so I can
include nice screenshots), red division lines & text, and green for the "block ROI" discussed next).  
...
#+ATTR_HTML: :align center :width 400
file:PhilaJ-dialog-bultDroi-Options.gif

For the most part, this dialog is precisely like the [[#sep-block][=Separate stamp multiple=]] tool -- one fewer color. Next the tool will ask the user
to identify the upper left stamp design like so:

#+ATTR_HTML: :align center :width 800
file:PhilaJ-bultDroi-LU.gif

Next the tool will ask the user to identify the lower right stamp design like so:

#+ATTR_HTML: :align center :width 800
file:PhilaJ-bultDroi-RL.gif

The generated design ROIs will then be displayed, and the user will be ask if the ROIs should be saved in the ROI manager:

#+ATTR_HTML: :align center :width 800
file:PhilaJ-bultDroi-res.gif

If the user instructs PhilaJ to save the ROIs, then ROIs will be created and saved in the ROI manager:

#+ATTR_HTML: :align center :width 400
file:PhilaJ-bultDroi-ROIs.gif
...
:END:
 ...
 - =<SID>= - Used to identify the stamp an ROI is associated with.
   ...
   - Recommendations
     - Sheets & if you know the sheet position: Use =p###= -- the "=p=" is for position.
