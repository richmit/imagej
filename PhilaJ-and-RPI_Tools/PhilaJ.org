...
*** Specialized Perforation Gauges, ROIs, etc...

PhilaJ uses several "dot" files to customize and extended behavior.  In the distribution you will find a directory called "=dot-philaj=" which contains the
set of customization files I use.  If you are just starting out, then you can start with these files.  Copy them into the [[#config-files][PhilaJ
...
| Key                   | Action                                                                        |
|-----------------------+-------------------------------------------------------------------------------|
| =[0]= (top row digit) | Locate and bring to the front a waiting ROI dialog                            |
...
| =[6]= (top row digit) | Reserved                                                                      |
| =[7]= (top row digit) | Reserved                                                                      |
| =[8]= (top row digit) | Reserved                                                                      |
| =[9]= (top row digit) | Reserved                                                                      |
...
| =[W]= (Uppercase)     | [[#cleaning-up-windows-n-files][=Close Image, PhilaJ, & Related Windows=]]    |
...
#+INCLUDE: "dot-philaj/perfs/Bari_Wolf_AMG.csv" src text
...
space]] is used for all PhilaJ color computations and reporting.  With CIELAB colors are described via three quantities: $L$, $A$, & $B$.  In addition, two
...
representing the "shade" of a color.  This is an /angle/ measured in degrees and always in the range $0$ to $360$.  I have been pretty successful using this
number to identify some difficult stamp shade varieties.  PhilaJ also reports another angle, $J$, which I call "Jue" -- in Spanish the "J" is frequently
pronounced as an "H". $J$ is simply $H$ centered at zero and ranging from $-180$ to $180$ -- logically it is equal to $H$; however, it is useful when
measuring colors with a hue angle distribution that covers a range around zero.
...
When activated, [[#measure-color][=Measure Color=]] menu option can produce reports in two windows:
  ...
  - A histogram of the one of the channels.  By default the option is "=H or J=" which will pick the channel with the lowest standard deviation -- this is an
    attempt to pick the "best" option between $H$ or $J$ that will deliver a nice Histogram.
...
When you first install Philaj, you may need to create this directory!
