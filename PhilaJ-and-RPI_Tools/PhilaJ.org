...
are used to label the stamp images with the position number from the original sheet from which the block came:
 - The *sheet* position of the upper left stamp -- if you don't know, set it to "1"
 - The width of the *sheet* from which the block came -- if you don't know, then set it to the number of columns in the block
Once the dialog is cleared, the tool will ask the user to define a "block ROI".  This ROI should define the outer edge of where we would cut the stamps (see
image below).
